...
class TransformationModelC:
    """A supply-use table to input-output table transformation object.
    From the supply-use table an  industry-by-industry input-output table
    based on fixed industry sales structure assumption is created. In the 'Eurostat Manual
    of Supply, Use and Input-Output Tables' this transformation model is called
    model C. The resulting input-output table may contain negative values. Only
    ...
    def check_io_transaction_matrix(self, rel_tol=default_rel_tol):
        ...
        g2 = np.transpose(self._sut.total_industry_input)
